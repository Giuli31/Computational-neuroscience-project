...
############################# COHERENCE ####################################
# By changing the parameter k that modulates the coherence of the input signal
# the color of the boxes will be a various mix of red and blue boxes

#----------- STILL ON WORKING --------------

tot_box = 9
k = 4   #total number of boxes
input_red = k 
input_blue = 9-k

    StateMachine.add('COHERENCE_MSG',
                     CBState(notify_user_cb('The color of the boxes is both blue and red to tune input coherence')),
                     {'succeeded': 'DELAY_ON_MOTION1'})
    ...
    StateMachine.add('ALTERNATED',
                for i in range(0,input_red,1):
                    string1 = 'Box'+str(i)
                    SetMaterialColorServiceState('string1',
                                          'body',
                                          'visual',
                                          'Gazebo/RedGlow'),

                for j in range(input_red,9,1):
                    string2 = 'Box'+str(j)
                    SetMaterialColorServiceState('string2',
                                          'body',
                                          'visual',
                                          'Gazebo/BlueGlow'),
                transitions={'succeeded': 'WAIT',
